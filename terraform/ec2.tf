...
resource "aws_launch_template" "app" {
  ...
  user_data = base64encode(<<-EOF
    ...
    # Clone application
    dnf install -y git
    git clone https://github.com/techstream/self-healing.git /opt/techstream
    cd /opt/techstream
    ...
    # Create systemd unit for easy control
    cat > /etc/systemd/system/techstream.service << 'UNIT'
    [Unit]
    Description=TechStream Self-Healing Demo
